## \file /src/ai/gemini/generative_ai (9).py
# -*- coding: utf-8 -*-
...
class GoogleGenerativeAI:
    ...
    def ask(self, 
        ...
        """
        if MODE in ('dev','debug'):pprint(f"""INPUT > {q}""",text_color='light_blue')
        self.history_file = self.history_dir / history_file if history_file else self.history_txt_file
        if MODE in ('dev','debug'):pprint(f"""HYSTORY FILE: > {self.history_file}""",text_color='light_green')
        try:
            history = read_text_file(self.history_file)
            complete_prompt = f"{history}\n* question *\n{q}\n* answer **\n"
            if MODE in ('dev','debug'):pprint(f"""PROMPT: > {complete_prompt}""",text_color='green')
            messages = [
            ...
            if not response:
                ...
                return 
            if MODE in ('dev','debug'):pprint(f"""RESPONSE: > {response}""", text_color='cyan')
